...
with seed(
  ...
  values
    (U&'\9752\8336', 'qing-tea',                '0-20 kcal',   0.0,  0.0, 0.0,  10.0, 'Tea base, almost zero calories.',            'Prefer no sugar or light sugar.',                    '["tea"]'::jsonb,                 '["light"]'::jsonb,         500.0, 35.0, 0.0, true),
    ...
    (U&'\7F8E\5F0F\5496\5561', 'americano-coffee','0-20 kcal',  0.0,  0.0, 0.0,  10.0, 'Black coffee base.',                           'No-sugar black coffee is the lowest-calorie choice.', '["coffee"]'::jsonb,              '["light"]'::jsonb,         500.0, 24.0, 0.0, true),
    (U&'\62FF\9435', 'latte-coffee',            '150-220 kcal',6.0, 13.0, 6.0,  80.0, 'Milk coffee drink.',                           'Keep sugar low and watch total daily fat.',          '["coffee","milk"]'::jsonb,       '["higher carbs"]'::jsonb,  500.0, 20.0, 0.2, true),
    (U&'\5361\5E03\5947\8AFE', 'cappuccino-coffee','130-200 kcal',6.0,11.0,5.0,75.0, 'Foamed milk coffee.',                          'No sugar or light sugar is preferred.',              '["coffee","milk"]'::jsonb,       '["light"]'::jsonb,         500.0, 20.0, 0.2, true),
    (U&'\6469\5361', 'mocha-coffee',            '180-260 kcal',5.0, 28.0, 8.0,  95.0, 'Chocolate coffee drink.',                      'If cutting calories, switch to unsweetened latte.',  '["coffee","cocoa"]'::jsonb,      '["higher carbs"]'::jsonb,  500.0, 28.0, 0.6, true),
    (U&'\53EF\53EF', 'cocoa-drink',             '190-250 kcal',4.0, 26.0, 6.0, 110.0, 'Cocoa drink with sugar already included.',     'Choose smaller cup size, especially at night.',      '["cocoa"]'::jsonb,               '["higher carbs"]'::jsonb,  500.0,  0.0, 1.0, false),
    ...
    (U&'\51AC\74DC\9752\8336', 'winter-melon-qing-tea','70-100 kcal',0.0,20.0,0.0,28.0,'Winter melon syrup with tea base.',          'Keep cup size smaller to control sugar intake.',     '["winter melon tea","green tea"]'::jsonb, '["higher carbs"]'::jsonb, 500.0, 0.0, 1.0, false),
    ...
    (U&'\7518\8517\9752\8336', 'sugarcane-qing-tea','95-135 kcal',0.0,25.0,0.0,22.0,'Sugarcane juice provides intrinsic sugar.',    'Keep cup size smaller when cutting calories.',       '["sugarcane","green tea"]'::jsonb, '["higher carbs"]'::jsonb, 500.0, 0.0, 1.0, false),
...
with alias_seed(food_name, alias_lang, alias) as (
  values
    ...
    (U&'\9752\8336', 'en', 'qing tea'),
    (U&'\9752\8336', 'en', 'qingcha'),
